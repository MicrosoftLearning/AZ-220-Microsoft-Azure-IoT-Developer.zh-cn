---
lab:
    title: '实验室 18：使用 Azure Defender for IoT 检测设备篡改情况'
    module: '模块 10：Azure Defender 和 IoT 安全'
---

# 使用 Azure Defender for IoT 检测设备篡改情况
...
![实验室 18 基础结构](media/LAB_AK_18-architecture.png)
...
如果这些资源不可用，请按照以下说明运行 **lab18-setup.azcli** 脚本，然后再前往练习 2。脚本文件包含在本地克隆作为开发环境配置（实验室 3）的 GitHub 存储库中。

> **备注**： 写入 **lab18-setup.azcli**  脚本，在 **Bash** shell 环境中运行 - 在 Azure Cloud Shell 中执行此操作最简便。
...
1. 在“文件选择”对话框中，导航到配置开发环境时下载的 GitHub 实验室文件的文件夹位置。
    ...
    * Allfiles
    ...
    lab18-setup.azcli 脚本文件位于实验室 18 的 Setup 文件夹中。

1. 选择 **lab18-setup.azcli** 文件，然后单击 **“打开”**。
...
1. 要验证是否上传了正确的文件，请输入以下命令：
    ...
    使用 `ls` 命令列出当前目录的内容。你应该会看到列出的 lab18-setup.azcli 文件。
...
1. 若要为此实验室创建一个包含安装脚本的目录，然后移至该目录，请输入以下 Bash 命令：
    ...
    这些命令将为此实验室创建一个目录，将 **lab18-setup.azcli** 文件放入该目录，然后将当前工作目录更改为该新目录。

1. 为确保 **“lab18-setup.azcli”** 具有执行权限，请输入以下命令：

    ```bash
    chmod +x lab18-setup.azcli
    ```

1. 在“Cloud Shell”工具栏上，请单击 **“打开编辑器”** （右侧的第二个按钮 - **{ }**）以启用对 lab18-setup.azcli 文件的访问。

1. 在 **“文件”** 列表中，要展开 lab18 文件夹并打开脚本文件，请先单击 **“lab18”**，再单击 **“lab18-setup.azcli”**。

    编辑器现在将显示 **lab18-setup.azcli** 文件的内容。
...
### 练习2：启用 Azure Defender for IoT 中心
...
#### 任务 1：启用 Azure Defender for IoT
...
1. 在左侧菜单中的“**Defender for IoT**”下，单击“**概览**”。
...
1. 花费片刻时间查看“安全概述”窗格上的内容。
    ...
    > **备注**： 载入 Azure Defender for IoT 的第一时间是无法立即检测到威胁的，在本实验室结束前，你将看到此“概览”窗格上报告威胁检测。

#### 任务 2：创建 Log Analytics
...
1. 返回到 **“设置” | “数据收集”** 页，在 **“工作区”** 下拉列表中，选择 **“log-az220-training-{your-id}”**
...
1. 在 **“资源组”** 下拉列表中，单击 **“rg-az220vm”**。
    ...
    > **备注**： 我们将使用一个资源组来跟踪和管理本课程中创建的所有虚拟机资源。如果 **rg-az220vm** 资源组还没有创建，请使用下面的说明立即创建：

    * 在 **“资源组”** 下拉列表中，选择 **“新建”**。
    * 在上下文菜单中的 **“名称”** 下，输入 **“rg-az220vm”**，然后单击 **“确定”**

    > **备注**： 你可能会遇到建议为每个 VM 创建一个单独的资源组的指导。为每个虚拟机创建一个单独的资源组可以帮助你管理添加到虚拟机的任何额外资源。但由于在本课程中 VM 的使用方式比较简单，因此为每个 VM 建立单独的资源组是不必要的，也是不实际的。
...
1. 在 **“可用性选项”** 下拉列表中，确保选中 **“无需基础架构冗余”**。
    ...
    > **提示**：
    > Azure 提供一系列的选项，用于管理应用程序的可用性和复原能力。将解决方案构建为使用可用性区域或可用性集中的已复制 VM，使应用和数据免受事件中心中断和维护事件的影响。在本实验室中，我们不需要任何高可用性功能。
...
1. 在 Azure 门户中，如有必要，导航到 IoT 中心的 **“IoT 设备”** 窗格。
    ...
    要打开 IoT 中心边栏选项卡的 **“IoT 设备”** 窗格，在左侧菜单中的 **“资源管理器”** 下，单击 **“IoT 设备”**。
...
1. 在边栏选项卡顶部附近的“概要”区域中，在 **“主机名”** 右边，单击 **“复制到剪贴板”**，然后保存该值以备后用。
    ...
    > **备注**： IoT 中心的主机名类似于：iot-az220-training-cah102119.azure-devices.net

### 练习 5：部署 Azure Defender for IoT C# 安全代理
...
* 通过使用 **azureiotsecurity** 模块孪生进行远程配置。如需了解更多信息，请参阅《配置 Azure Defender for IoT 代理》。
...
1. 在 **“名称”** 下，单击 **“vm-az220-training-edge0002-{your-id}”**。
    ...
    新创建虚拟机 (**vm-az220-training-edge0002-{your-id}**) 的“概览”窗格现在应该已打开。
...
1. 确认你是否拥有可用的 **vm-az220-training-edge0002-{your-id}** 设**备的主键**值。
...
1. 在 Cloud Shell 命令提示符下，输入以下命令：
    ...
    此命令将使用 **vm-az220-training-edge0002-{your-id}** 设备**的主键**创建设备身份验证类型文件。

    > **备注**： 要检查是否向文件中添加了正确的主键，请使用 `nano s.key` 命令打开文件。检查设备**的主键**是否在文件中。要退出 nano 编辑器，请按住 `Ctrl` 和 `X`。按住 `shift` 和 `Y`保存文件。然后按 Enter。
...
1. 在 Cloud Shell 命令提示符下，要将最新版本的 C# 安全代理下载到设备，请输入以下命令：
    ...
    > **备注**： 请注意，上述命令面向 Ubuntu Server 18.04 LTS
...
1. 在 Cloud Shell 命令提示符下，自定义并输入以下命令：
    ...
    > **备注**： 确保指定你的 IoT 中心主机名，而不是列出的那个主机名

    > **重要说明**：
    > 确保为 `-hn` 切换值使用完整的 IoT 中心主机名，即 **iot-az220-training-ab200213.azure-devices.net**。
...
1. 在 Cloud Shell 命令提示符下，请输入以下命令检查 Azure Defender for IoT 代理的部署状态。
    ...
    > **备注**：如果 Azure Defender for IoT 代理未运行或未处于活动状态，请查看[为 Linux 部署 Defender for IoT 基于 C# 的安全代理](https://docs.microsoft.com/zh-cn/azure/defender-for-iot/device-builders/how-to-deploy-linux-cs)。常见问题是可能会造成服务 **Active: activating** 的键值错误或未指定完整的 IoT 中心主机名。

1. 在 Azure 门户中，导航回 IoT 中心边栏选项卡，然后打开 **vm-az220-training-edge0002-{your-id}** 设备边栏选项卡。

    打开 IoT 中心边栏选项卡，在 **“资源管理器”** 下的导航菜单中，单击 **“IoT 设备”** ，然后单击 **“vm-az220-training-edge0002-{your-id}”**。
...
对于完全了解其预期设备行为的客户，Azure Defender for IoT 可将这种了解转化为设备行为策略，并在背离预期的正常行为时发出警报。
...
1. 要将设备添加到**默认**安全组，请在 **“版本”** 和 **“属性”** 字段之间插入以下 JSON：
...
1. 打开 **Program.cs** 文件。
    ...
    粗略地看一下就会发现，**CaveDevice** 应用程序与前面的实验室中使用的应用程序非常相似。这个版本增加了延迟，因此每 10 秒就会发送一次消息：`await Task.Delay(10000);`
...
### 练习 9：查看 Azure Defender for IoT 警报
...
#### 任务 1：查看 Azure Defender for IoT 仪表板
