...
1. 在 Web 浏览器中导航到 [portal.azure.com](http://portal.azure.com)，打开 Azure 门户。
...
1. 也可以按照相同的过程将以下服务添加到收藏列表：**设备预配服务**、**逻辑应用**、**流分析作业**，**以及存储帐户**。

    > **备注**： 你可以通过单击所选服务的星形图标将该服务从收藏服务列表中移除。
...
1. 在 **“帮助 + 支持”** 边栏选项卡上，请注意以下四个磁贴：“入门”__、“文档”__、“计费 FAQ”__和“支持计划”__。

    通过“帮助 + 支持”边栏选项卡可访问大量资源。你可能希望稍后再来进一步探讨这个问题。

1. 在 **“帮助 + 支持”** 边栏选项卡上，单击 **“计费 FAQ”**

    应会打开一个显示 Azure 计费文档的新浏览器选项卡。

1. 花一些时间扫描 **“使用 Azure 计费和成本管理防止意外收费”** 页面的内容。 

   *如果*正在使用付费 Azure 订阅，并且由你负责计费（你是帐户管理员），可以设置成本警报来帮助管理计费。
...
1. 在 **“资源组”** 文本框中，输入 **“rg-az220”**
    ...
    资源组的名称必须在订阅中具**有唯**一性。如果尚未使用你输入的名称并确认资源组命名规则，则会出现一个绿色的对勾。
...
1. 在 **“区域”** 下拉菜单中，选择你附近的区域。  
    ...
    你需要为资源组提供一个位置，因为资源组存储有关资源的元数据，并且充当将在资源组中创建新资源的默认位置。出于合规目的，你可能需要指定该元数据的存储位置。一般情况下，建议指定大部分资源将驻留的位置。使用相同的位置可以简化用于管理资源的模板。
...
1. 在已命名的资源组列表中，单击刚创建的 **rg-az220** 资源组左侧的框。
    ...
    > **备注**： 不建议你在新边栏选项卡中将资源组打开，只要将其选中即可（左侧的对勾）。
